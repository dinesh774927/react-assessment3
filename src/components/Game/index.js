...
class Game extends Component {
  state = {tabs: 'FRUIT', score: 0, timer: 60, isRight: true, random: 0}
  ...
  time = () => {
    ...
    if (timer === 0) {
      ...
      this.setState({isRight: false, score})
    } else {
      this.setState(prev => ({
        timer: prev.timer - 1,
      }))
  ...
  check = id => {
    ...
    if (id === imagesList[random].id) {
      ...
      this.setState(prev => ({random: num, score: prev.score + 1}))
    } else {
      clearInterval(this.timeId)
  ...
  render() {
    const {tabs, timer, random, isRight, score} = this.state
    const {tabsList} = this.props
    const image = imagesList[random].imageUrl
    const newList = imagesList.filter(each => each.category === tabs)
    return (
      <div>
        <ul className="ul-container">
          ...
          {isRight ? (
            <div className="container">
              <li>
                <img className="match-img" alt="match" src={image} />
                <ul className="ul">
                  {tabsList.map(each => (
                    <TabItems
                      button={tabs}
                      func={this.tabsChange}
                      item={each}
                      key={each.tabId}
                    />
                  ))}
              ...
              <li>
                <ul className="ul">
                  {newList.map(each => (
                    <Items func={this.check} items={each} key={each.id} />
                  ))}
                </ul>
              </li>
            </div>
